...
Classify the offer into one of the archetypes defined in `_profile.md` (which overrides `_shared.md`). If it's a hybrid, indicate the 2 closest. This determines:
...
Table with:
- Detected archetype
- Domain (marketing / pre-sales / post-sales / AI-native / enterprise SaaS, etc.)
- Function (build narrative / generate pipeline / close deals / onboard / deploy)
- Seniority
...
**Adapted to the archetype (per `_profile.md`):**
- If PMM → prioritize launches, positioning wins, sales enablement
- If Growth / Demand Gen → prioritize pipeline generated, funnel metrics, channel expertise
- If AI PMM → prioritize ability to speak to both developer and exec audiences + AI fluency
- If Pre-Sales SC / SE → prioritize deals influenced, demo skill, technical wins
- If Post-Sales / Implementation → prioritize accounts onboarded, adoption metrics, NRR
- If AI SC / Forward Deployed → prioritize client-facing AI delivery and integration design

Include a **gaps** section with mitigation strategy for each. For each gap:
...
1. **Level detected** in the JD vs **natural level of the candidate** for that archetype
2. **"Sell senior without lying" plan**: specific phrases adapted to the archetype, concrete achievements to highlight, how to position prior experience as an advantage
3. **"If they downlevel me" plan**: accept if comp is fair, negotiate a 6-month review, clear promotion criteria

## Block D — Comp and Demand

Use WebSearch for:
- Current salaries for the role (Glassdoor, Levels.fyi, Pave, RepVue for SC)
- Company compensation reputation
- Demand trend for the role

Table with data and cited sources. If data is missing, say so rather than invent.

For **SC / SE** roles, always surface **OTE** (base + variable) and expected split. For **Marketing** roles, surface base + bonus + equity where known.
...
**Selected and framed per the archetype (per `_profile.md`):**
- PMM → emphasize launch outcomes, narrative, cross-functional wins
- Growth / Demand Gen → emphasize pipeline lifts, experimentation, attribution
- AI PMM → emphasize translating technical concepts for non-technical buyers
- Pre-Sales SC / SE → emphasize discovery, technical wins, complex deal support
- Post-Sales / Implementation → emphasize time-to-value, adoption, escalation handling
- AI SC / Forward Deployed → emphasize rapid deployment, prompt/eval work, customer co-design

Also include:
- 1 recommended case study (which of their projects to present and how)
- Red-flag questions and how to answer them (e.g., "why did you leave X?", "do you have direct reports?")
...
- Does it name specific technologies, frameworks, tools, or methodologies?
...
- Does the role make sense for this company's business stage?
...
(only if score >= 4.5 — drafts of responses to the application form)
...
**ALWAYS** write a TSV in `batch/tracker-additions/{num}-{company-slug}.tsv` (NEVER edit `data/applications.md` directly to add entries). See `CLAUDE.md` for the TSV column order. The merge script handles integration.
